...
contract CandidateManager is Ownable {
    ...
    struct CandidateState {
        uint256 candidateType;
        bool isCandidate;
    }

    mapping(address => CandidateState) CandidatesState;

    event NewCandidate(address _candidate, string _name, address _coinbase, uint256 _type);
    ...
    function newCandidate(string memory  _candidateName, address _coinbase) public onlyOwner {
        ...
        emit NewCandidate(address(_candidate), _candidateName, _coinbase, 1);
    }
    ...
    function addCandidate(address _can, string memory _candidateName, address _coinbase, uint256 _type) public onlyOwner {
        require(_type > 0);
        candidates.push(_can);
        CandidatesState[_can] = CandidateState({
                    candidateType: _type,
                    isCandidate: true});
        emit NewCandidate(address(_can), _candidateName, _coinbase, _type);
    }

    function getCandidateType(address _can) public view returns (uint256) {
        return CandidatesState[_can].candidateType;
    }
}
