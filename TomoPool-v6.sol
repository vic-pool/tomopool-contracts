...
contract CandidateContract {
    ...
    bool public isRequireReferralLock25k = true;
    ...
    constructor (string memory _candidateName, address _coinbase) public {
    ...
    modifier ownerLock25k() {
        if (isRequireReferralLock25k) {
            require(getCurrentStakerCap(referralAddress) >= 25000 ether);
        }
        _;
    }
    function setRequireReferralLock25k(bool _lock) public onlyTeam {
        isRequireReferralLock25k = _lock;
    }
    ...
    function stake() public payable validMaxCap(msg.value) ownerLock25k {
    ...
    function transferStake(address to, uint256 _amount) external onlyStaker {
        if (msg.sender == referralAddress && isRequireReferralLock25k) {
            require(getCurrentStakerCap(msg.sender).sub(_amount) >= 25000 ether);
        }
